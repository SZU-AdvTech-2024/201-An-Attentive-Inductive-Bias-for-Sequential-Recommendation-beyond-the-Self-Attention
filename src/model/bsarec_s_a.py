...
class BSARecModel_S_A(SequentialRecModel):
    def __init__(self, args):
        super(BSARecModel_S_A, self).__init__(args)
        self.args = args
...
class BSARecBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        dsp, gsp = self.layer(hidden_states, attention_mask)
        feedforward_output_d = self.feed_forward_d(dsp)
        feedforward_output_g = self.feed_forward_g(gsp)
        fused_output = self.fusion_module(feedforward_output_d, feedforward_output_g)
        return fused_output
