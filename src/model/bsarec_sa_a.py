...
class BSARecModel_SA_A(SequentialRecModel):
    def __init__(self, args):
        super(BSARecModel_SA_A, self).__init__(args)
        self.args = args
...
class BSARecBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        sa, dsp = self.layer(hidden_states, attention_mask)
        feedforward_output_sa = self.feed_forward_sa(sa)
        feedforward_output_d = self.feed_forward_d(dsp)
        fused_output = self.fusion_module(feedforward_output_sa, feedforward_output_d)
        return fused_output
