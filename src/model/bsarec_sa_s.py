...
class BSARecModel_SA_S(SequentialRecModel):
    def __init__(self, args):
        super(BSARecModel_SA_S, self).__init__(args)
        self.args = args
...
class ModalityFusion(nn.Module):
    def __init__(self):
        ...
        # 初始化权重参数，可以是可学习的
        self.weight_d = nn.Parameter(torch.tensor(0.5))
        self.weight_g = nn.Parameter(torch.tensor(0.5))

    def forward(self, feedforward_output_d, feedforward_output_g):
        # 确保权重之和为1
        weight_sum = self.weight_d + self.weight_g
        normalized_weight_d = self.weight_d / weight_sum
        normalized_weight_g = self.weight_g / weight_sum
        
        # 加权求和
        fused_output = normalized_weight_d * feedforward_output_d + normalized_weight_g * feedforward_output_g
        return fused_output
...
class BSARecBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        sa, gsp = self.layer(hidden_states, attention_mask)
        feedforward_output_sa = self.feed_forward_sa(sa)
        feedforward_output_g = self.feed_forward_g(gsp)
        fused_output = self.fusion_module(feedforward_output_sa, feedforward_output_g)
        return fused_output
